<?php

namespace App\Http\Controllers\API\V1\Site\Quiz;

use App\Http\Controllers\API\V1\Site\AdminAPIBaseController;
use App\Http\Requests\API\V1\Site\Quiz\StoreQuizRequest;
use App\Http\Requests\API\V1\Site\Quiz\UpdateQuizRequest;
use App\Http\Resources\API\V1\Site\Quiz\QuizCollection;
use App\Http\Resources\API\V1\Site\Quiz\QuizResource;
use App\Models\V1\Quiz\Quiz;
use Illuminate\Http\Request;

class QuizController extends AdminAPIBaseController
{
    public function index(Request $request)
    {
       $quizzes=Quiz::applyTrashFilterAble()
                ->applyKeywordSearchAble()
                ->applySortAble()
                ->applyPaginateAble();
       return new QuizCollection($quizzes);
    }


    public function store(StoreQuizRequest $request)
    {
        // $this->makeFakeLogin();
        $created_by_id = auth()->user()->id;
        //Create a quiz data
        $quiz=new Quiz();
        $quiz->fill($request->all());
        $quiz->created_by_id=$created_by_id;
        $quiz->save();
        return new QuizResource($quiz);
    }

    public function show($quizId)
    {
        $quiz=Quiz::with('questions.choices')->findOrFail($quizId);
        return new QuizResource($quiz);
    }

    public function update(UpdateQuizRequest $request,$quizId)
    {
        $quiz=Quiz::findOrFail($quizId);
        $quiz->fill($request->all());
        $this->authorize('update',$quiz);
        $quiz->save();
        return new QuizResource($quiz);
    }



    public function trash($quizId)
    {
        $quiz=Quiz::findOrFail($quizId);
        $this->authorize('trash',$quiz);
        $quiz->delete();
        return response()->noContent();
    }

        //restore data
    public function restore($quizId)
    {
        $quiz=Quiz::withTrashed()->findOrFail($quizId);
        $this->authorize('restore',$quiz);
        $quiz->restore();
        return new QuizResource($quiz);
    }

    //PERMANENT DELETE
    public function destroy($quizId)
    {
        $quiz=Quiz::withTrashed()->findOrFail($quizId);
        $this->authorize('forceDelete',$quiz);
        $quiz->forceDelete();
        return response()->noContent();
    }
}
