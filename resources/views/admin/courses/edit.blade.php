                        ...
                        @if(session()->has('success'))
                            <div class="row">
                                <div class="alert alert-success">
                                    ...
                                    <strong><a href="{{route('web.admin.courses.show', session('rID'))}}">Click</a> here to show</strong>
                                </div>
                            </div>
                        @endif
                        <div class="card-body">
                            <form class="form-horizontal" action="{{route('web.admin.courses.update.all',$course->id)}}" method="post" enctype="multipart/form-data">
                                @csrf
                                @include('errors.form-error', ['errors'=>$errors])
                                <div class="form-group row">
                                    <label class="col-md-3 col-form-label" for="text-input">Name</label>
                                    <div class="col-md-9">
                                        <input class="form-control" id="text-input" type="text" name="name" value="{{old('name',$course->name)}}"
                                               placeholder="Text">
                                    </div>
                                </div>
                                <div class="form-group row">
