    ...
    <div class="col-md-8">
        <div class="btn-toolbar float-right" role="toolbar"
            ...
            <div class="btn-group" role="group" aria-label="Third group">
                @if(request('trashed'))
                    <a href="{{route('web.admin.quiz-questions.index')}}"
                       class="btn btn-dark"
                       type="button">Without Trashed</a>
                @else
                    <a href="{{route('web.admin.quiz-questions.index',['trashed' => true])}}"
                       class="btn btn-dark"
                       type="button">View Trashed</a>
                @endif
                <a href="{{ route('web.admin.quiz-questions.create') }}"
                ...
                <div class="form-group col-md-3 mb-0">
                    <select name="question_id" id="question_id" class="form-control filterable" required>
                        ...
                        @foreach($quizzes as $quiz)
                            <option value="{{ $quiz->id }}"  @if( request('quiz_id')== $quiz->id) selected @endif >{{ $quiz->name }}</option>
                        @endforeach
                    </select>
                </div>
                 <div class="form-group col-md-3 mb-0">
